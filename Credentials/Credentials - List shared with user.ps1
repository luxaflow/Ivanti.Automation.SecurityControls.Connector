...
for ($i = 0; $i -le $result.sharedWith.Count-1; $i++) 
{
  if ($result.sharedWith[$i] -eq $result.ownerName) 
  {
    Write-Host $result.sharedWith[$i] '(Owner)'
  }
  else 
  {
    Write-Host -Object $result.sharedWith[$i]
  }
}
