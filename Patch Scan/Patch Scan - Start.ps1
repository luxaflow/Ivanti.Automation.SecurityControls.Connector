...
# Jun 2020 - Update to use CredentialID and not CredentialId
#
...
if (-not $PatchTemplateID) {
    ...
    try {
        ...
        # Dig into the exception to get the Response details.
        # Note that value__ is not a typo.
        Write-Host "Error (GetPatchTemplateID)"
        Write-Host "StatusCode:" $_.Exception.Response.StatusCode.value__ 
        Write-Host "StatusDescription:" $_.Exception.Response.StatusDescription
        Write-Host "Error Message:" $_.ErrorDetails.Message
        exit(1)
    ...
    if ($found -eq 0) {
        #no match found
        write-host "Error: Patch Template not found"
        exit (1)
        }
...
if (-not $MachineGroupID -and $ScanMachineOrGroup -eq "Group") {
    ...
    try {
        ...
        # Dig into the exception to get the Response details.
        # Note that value__ is not a typo.
        Write-Host "Error (GetMachineGroupID)"
        Write-Host "StatusCode:" $_.Exception.Response.StatusCode.value__ 
        Write-Host "StatusDescription:" $_.Exception.Response.StatusDescription
        Write-Host "Error Message:" $_.ErrorDetails.Message
        exit(2)
    ...
    if ($found -eq 0) {
        #no match found
        write-host "Error: Machinegroup not found"
        exit (2)
        }
...
if ($ScanMachineOrGroup -eq "Group") { #We are going to scan a MachineGroup
    $patchScanBody =
    @{
...
} else { #We are going to scan a machine
    $patchScanBody =
    @{
...
try {
    ...
    # Dig into the exception to get the Response details.
    # Note that value__ is not a typo.
    Write-Host "Error (InvokePatchScan)"
    Write-Host "StatusCode:" $_.Exception.Response.StatusCode.value__ 
    Write-Host "StatusDescription:" $_.Exception.Response.StatusDescription
    Write-Host "Error Message:" $_.ErrorDetails.Message
    exit (4)
